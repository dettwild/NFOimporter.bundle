	...
	def update(self, metadata, media, lang):
		...
		if nfoFile:
			...
			if nfoTextLower.count('<movie') > 0 and nfoTextLower.count('</movie>') > 0:    #contains the main tags --> lools like a KODI nfo file
				...
				#========================= WATCHED state (only if true, NOT setting sth to unwatched if XML in nfo indicates that)
					...
					if myval == "true":
						...
						if myviewcount == 0:
							...
							try:
								...
								#HTTP.Precache(myurl)
								#myreply = 
								myans=HTTP.Request(myurl2, headers=myheader)
								#Log(HTTP.Request(myurl)) #, cacheTime = 2) #,sleep=0.2)
								
								#replyxml = XML.ElementFromString(myreply)
								#HTTP.ClearCache()
								...
								pass
							try: replyxml = XML.ElementFromString(myans)
							except:
				...
				# trying to obtain the current agent token to remain felxible and avoid hardcoding
				try: 
